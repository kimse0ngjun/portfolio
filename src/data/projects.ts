...
export const projects = [
  ...
  },
  {
    ...
    technologyGroups: [
      { label: "Backend", items: ["Java 21", "Spring Boot 3.5.5", "Spring Security", "JWT", "Spring Data JPA", "MyBatis"] },
      { label: "Frontend", items: ["React", "Vite", "React Router"] },
      { label: "Data & AI", items: ["MySQL", "Tesseract OCR", "Gemini API"] },
    ],
    overviewDescription: "제품명과 LOT 번호로 의약품 리콜 정보를 검색하고, 이미지 OCR과 생성형 AI 요약을 활용해 필요한 정보를 확인할 수 있도록 구현한 개인 프로젝트입니다.",
    overviewItems: [
      { label: "프로젝트 형태", value: "Personal Project" },
      { label: "담당 영역", value: "Backend · Frontend" },
      { label: "서비스 유형", value: "의약품 리콜 정보 검색" },
      { label: "의약품 데이터", value: "자료 확인 중" },
      { label: "리콜 배치 데이터", value: "자료 확인 중" },
    ],
    contributionDescription: "공개 저장소의 README와 커밋 기록을 기준으로 Backend와 Frontend 전반을 구현한 개인 프로젝트입니다.",
    contributions: [
      ...
      { title: "AI 리콜 정보 요약", description: "조회한 리콜 결과로 Prompt를 구성하고 Gemini API 응답을 파싱해 전체 회수 현황, 주요 사유와 사용자 주의사항을 요약하는 기능을 구현했습니다." },
      { title: "인증 및 조회 이력 관리", description: "Spring Security와 JWT 인증을 구성하고 제품명·LOT·이미지 조회 이력을 저장·조회·삭제하는 API와 마이페이지를 구현했습니다." },
      { title: "React 화면 및 API 연동", description: "검색 유형별 입력과 결과 화면, 리콜 상세 정보, 사용자 이력 화면을 Backend API와 연결하고 이력 목록에 클라이언트 페이지네이션을 적용했습니다." },
    ],
    stackDescription: "공개 저장소의 현재 구성 파일과 구현 코드에서 확인되는 기술입니다.",
    architectureDescription: "React 클라이언트가 JWT와 함께 Spring Boot REST API를 호출하며, Backend는 MySQL 조회·이력 저장, Tesseract OCR 처리와 Gemini API 요약 요청을 담당합니다.",
    ...
    troubleshootingDescription: "공개 저장소의 수정 커밋에서 확인되는 Frontend와 Backend 연동 문제를 정리했습니다.",
    troubleshooting: [
      { title: "이미지 조회 API의 multipart 필드 불일치", problem: "이미지 리콜 조회와 이미지 조회 이력 저장 요청에서 업로드 파일이 Backend에 올바르게 전달되지 않았습니다.", cause: "두 Backend API가 각각 요구하는 multipart 필드 이름이 달랐지만 Frontend가 `file`과 `image`를 반대로 전송하고 있었습니다.", solution: "리콜 이미지 조회에는 `image`, 이미지 검증 이력 저장에는 `file`을 사용하도록 각 FormData 필드와 호출 흐름을 API 계약에 맞게 수정했습니다.", results: ["이미지 조회와 이력 저장 요청 형식 일치", "OCR 조회 후 이력 저장 흐름 연결"] },
    ...
    links: [
      { label: "GitHub Repository", url: "https://github.com/kimse0ngjun/medicine-platform", description: "새 창에서 Medicine Platform 공개 저장소를 엽니다.", variant: "primary" },
    ],
    outcomes: [
      ...
      "리콜 조회 결과를 Prompt로 구성하고 외부 생성형 AI API 응답을 서비스 화면에 제공하는 연동 흐름을 구현했습니다.",
      "JWT 인증과 조회 이력의 저장·조회·삭제, React 마이페이지를 연결하며 Backend와 Frontend의 데이터 계약을 조정했습니다.",
    ],
  },
  {
    slug: "specmate",
